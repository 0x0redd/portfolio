...
const FolderFloat: React.FC<FolderFloatProps> = ({
  ...
  const startPhysics = useCallback(() => {
    ...
    const tick = (now: number) => {
      ...
      s.bodies.forEach((b, i) => {
        ...
        const ph = (b as Matter.Body & { plugin: { phase: number } }).plugin
          .phase;
  ...
  const move = (e: PointerEvent<HTMLButtonElement>, i: number) => {
    ...
    const x = Math.min(
      z.right - bw / 2,
      Math.max(z.left + bw / 2, p.x + d.dx)
    );
    const y = Math.min(
      z.bottom - bh / 2,
      Math.max(z.top + bh / 2, p.y + d.dy)
    );
    Body.setVelocity(b, {
  ...
  return (
    ...
    >
      <style>{STYLE}</style>
      {/* Soft glow behind bubbles + rising from the folder */}
      <div
        aria-hidden
        ...
        style={{
          ...
          background: `
            ...
            radial-gradient(ellipse 50% 40% at 50% 35%, color-mix(in srgb, var(--ff-back) 40%, transparent) 0%, transparent 75%)
          `,
          filter: "blur(22px)",
        }}
      />
      ...
      <div className="relative z-[2] [width:var(--ff-w)] [height:var(--ff-h)]">
        {/* Folder body glow */}
        <span
          aria-hidden
          className="pointer-events-none absolute -inset-6 z-0 rounded-[28px] opacity-80 blur-2xl transition-opacity duration-500 group-data-[open]:opacity-100"
          style={{
            background:
              "radial-gradient(ellipse at 50% 70%, color-mix(in srgb, var(--ff-front) 65%, transparent), transparent 68%)",
          }}
        />
        <span
